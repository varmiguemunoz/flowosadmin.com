...
2. Produce the plate. With the API fallback: `.kiro/skills/impeccable/scripts/impeccable generate-image --plate <id> --quality high` does the whole step (crop as reference, the spec's plate prompt, output size chosen from the region's aspect, the file written to its plate path, prompt embedded, and the plate scored against the crop). With a harness-native image tool: use the crop as the input image and `.kiro/skills/impeccable/scripts/impeccable comp-spec --plate-prompt <id>` as the prompt, write the result to the plate path, then run `.kiro/skills/impeccable/scripts/impeccable embed-prompt <plate> --prompt "<the exact prompt>"`.
